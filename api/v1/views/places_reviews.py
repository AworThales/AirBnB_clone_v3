...
def list_reviews_of_place(place_id):
    ...
    place_objects = [obj.to_dict() for obj in all_placeses if obj.id == place_id]
    if place_objects == []:
        abort(404)
    list_reviewing = [obj.to_dict() for obj in storage.all("Review").values()
                    if place_id == obj.place_id]
    return jsonify(list_reviewing)
...
def get_review(review_id):
    ...
    if review_objects == []:
        abort(404)
    return jsonify(review_objects[0])
...
def delete_review(review_id):
    ...
    if review_objects == []:
        abort(404)
    review_objects.remove(review_objects[0])
